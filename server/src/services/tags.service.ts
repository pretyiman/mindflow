...
export async function createTag(mapId: string, data: { name: string; color?: string }) {
  try {
    return await prisma.tag.create({ data: { mapId, ...data } });
  } catch (err: any) {
    if (err?.code === 'P2002') {
...
export async function deleteTag(id: string, force: boolean) {
  await getTagOrThrow(id);
